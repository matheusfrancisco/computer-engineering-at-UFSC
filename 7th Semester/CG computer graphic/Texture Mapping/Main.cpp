...
void init (const char * imgDir, int textureId)
{
    ...
    // posicao fonte luz
    GLfloat posicaoLuz[4]={largura, altura, comprimento + 10, 1.0};
...
void draw(void)
{
    ...
    drawCube((quadrado.x*20.0 + 10.0)/2, (quadrado.y*20.0 + 10.0)/2, (quadrado.z*20.0 + 10.0)/2, 20.0f/2);
            ...
            for(k = 0; k < comprimento; k++) {
               // peca igual a um significa que o bloco esta parado e nao se mechera mais
                if(matrix[i][j][k] == 1) {
                    ...
                    drawCube((j*20.0 + 10.0)/2, (i*20.0 + 10.0)/2, (k*20.0 + 10.0)/2, 20.0f/2); 
                }
            }
        }
    }   
    ...
    if(quadrado.y == 0) { // se o bloco esta na ultima posicao
        matrix[quadrado.y][quadrado.x][quadrado.z] = 1;
        quadrado.x = largura/2;
        quadrado.z = comprimento/2;
        quadrado.y = altura - 1;
    } else if(matrix[quadrado.y - 1][quadrado.x][quadrado.z] == 1) { // se tem um bloco em baixo dele 
        matrix[quadrado.y][quadrado.x][quadrado.z] = 1; // estaciona o bloco que estava vindo em movimento
        quadrado.x = largura/2;
        quadrado.z = comprimento/2;
        quadrado.y = altura - 1;
    } else {
...
void keyboard(int Key, int x, int y) {
    switch(Key) {
        case GLUT_KEY_UP: // para a posicao inversa a coordenada Z negativa profundidade
            if(quadrado.z > 0) {
                quadrado.z--;
            }
            break;
        case GLUT_KEY_DOWN: // para a posicao positiva a coordenada Z
            if(quadrado.z < comprimento -1) {
                quadrado.z++;
            }
            break;
        case GLUT_KEY_LEFT: // para a posicao negativa a corodenada X ( largura)
            if(quadrado.x > 0) {
                quadrado.x--;
            }
            break;
        case GLUT_KEY_RIGHT: // para a posicao positiva a coordenada X largura
            if(quadrado.x < largura-1) {
                quadrado.x++;
            }
            break;
    }
